...
- `run(String, String)`メソッドの引数で入力ファイルと出力ファイルのファイル名それぞれ指定する．
...
## 構文解析
### 仕様
`Parser.java`は，Pascal風プログラムを字句解析した結果(tsファイル)を入力とし，プログラムが構文的に正しいことを判定する構文解析器である．構文解析器の仕様は以下の通りである．
- `run(String)`メソッドの第1引数で指定されたtsファイルを読み込み，構文解析を行う．
...
## 意味解析
### 仕様
`Checker.java`は，Pascal風プログラムを字句解析した結果(tsファイル)を入力とし，プログラムが意味的に正しいことを判定する意味解析器である．意味解析器の仕様は以下の通りである．
- `run(String)`メソッドの第1引数で指定されたtsファイルを読み込み，意味解析を行う．
  - 意味的に正しい場合，文字列`OK`を返す．
  - 意味的に正しくない場合，`Semantic error: line`という文字列とともに最初に検出された意味エラーの行番号を出力する．
- 複数の誤りが含まれる場合は，最初に検出された誤りのみを出力する．
- 意味解析の前に必ず構文解析を行い，構文エラーがあればエラーメッセージを返す．メッセージの内容は構文解析のものと同様である．
- 入力ファイルが存在しない場合，`File not found`という文字列を出力する．
...
- `run(String, String)`メソッドの第1引数と第2引数で入力ファイルと出力ファイルのファイル名をそれぞれ指定する．
...
- 構文的・意味的な誤りを発見した場合はエラーメッセージを返し，casファイルを生成してはいけない．
  - 構文的な誤りを発見した場合，`Syntax Error: line`という文字列とともに最初に検出された構文エラーの行番号を出力する．
  - 意味的な誤りを発見した場合，`Semantic Error: line`という文字列とともに最初に検出された構文エラーの行番号を出力する．
  - 意味的・構文的な誤りがどちらも含まれる場合は，構文的な誤りを優先して出力する．
...
#### lib.casの不使用
`lib.cas`を使用しない代わりに，`lib.cas`に登録されているサブルーチンの中で必要最小限のものがcasファイルに記載されるようになっている．つまり，到達不可能なサブルーチンを削除している．
